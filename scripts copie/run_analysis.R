...
source("/Users/mickaeleskinazi/Documents/GitHub/metaanalysis-psychedelics/scripts/analysis_plots_by_ae_overlay.R")

plot_dr_overlay_per_ae(
  preds           = dr_ae$preds,
  models          = dr_ae$models,
  outdir          = file.path(ROOT, "master/dr_overlay_by_ae"),
  min_molecules   = 2,
  normalize_dose  = TRUE
)

plot_forest_overlay_per_ae(
  es              = es,
  outdir          = file.path(ROOT, "master/forest_overlay_by_ae"),
  min_k           = 2,
  min_molecules   = 2
)
